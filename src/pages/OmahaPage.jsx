...
function createDefaultSettings(players) {
    return {
        buyIn: 200000,
        rankPoints: Object.fromEntries(players.map((player, idx) => [idx + 1, Math.max(players.length - idx, 0)])),
        rankPointsCustomized: false,
        blindLevels: [
            { id: 1, level: 1, blind: "10/20", duration: "25'", note: "" },
            { id: 2, level: 2, blind: "20/40", duration: "25'", note: "" },
            { id: 3, level: 3, blind: "30/60", duration: "25'", note: "" },
            { id: 4, level: 4, blind: "40/80", duration: "25'", note: "" },
            { id: 5, level: 5, blind: "60/120", duration: "25'", note: "" },
            { id: 6, level: 6, blind: "100/200", duration: "15'", note: "" },
            { id: 7, level: 7, blind: "150/300", duration: "15'", note: "" },
            { id: 8, level: 8, blind: "200/400", duration: "15'", note: "" }
        ]
    };
}
...
export default function OmahaPage({ players: seedPlayers }) {
    ...
    const [settings, setSettings] = useState({
        ...
        blindLevels: normalizeBlindLevels(initialData.settings?.blindLevels ?? [])
    });
    ...
    const profitByRound = useMemo(
        () => computeProfitByRound(rows, players, settings, "omaha"),
    ...
    const syncRowsWithPlayers = (nextPlayers) => {
        ...
        setSettings((prev) => ({
            ...prev,
            rankPoints: Object.fromEntries(
                nextPlayers.map((_, idx) => [idx + 1, prev.rankPoints?.[idx + 1] ?? Math.max(nextPlayers.length - idx, 0)])
            )
        }));
    };
    ...
    const summary = useMemo(() => {
        ...
        const totalPot = playerRows.reduce((sum, row) => sum + row.buyIn, 0);
        const totalPoints = playerRows.reduce((sum, row) => sum + row.points, 0);
        const withNet = playerRows.map((row) => ({
            ...row,
            net: row.rawProfit
        }));
        const check = withNet.reduce((sum, row) => sum + row.net, 0);
        return { players: withNet, totalPot, totalPoints, check };
    ...
    const getSummaryText = () =>
        [
            ["Người chơi", "Pts", "Ses", "Rb", "Buy-in", "Net"].join("\t"),
            ...
            ["Tổng pot", Math.round(summary.totalPot)].join("\t"),
    ...
    const resetWorkspace = () => {
        ...
        if (!confirmed) return;
        downloadJson(`omaha-auto-backup-before-reset-${new Date().toISOString().replace(/[:.]/g, "-")}.json`, getBackupData());
    ...
    const exportBackup = () => {
        downloadJson(`omaha-backup-${new Date().toISOString().slice(0, 10)}.json`, getBackupData());
    ...
    const importBackup = async (event) => {
        ...
        try {
            ...
            downloadJson(`omaha-auto-backup-before-import-${new Date().toISOString().replace(/[:.]/g, "-")}.json`, getBackupData());
    ...
    const addBlindLevel = () => {
        setSettings((prev) => {
            ...
            return {
                ...
                blindLevels: [
                    ...prev.blindLevels,
                    { id: nextId, level: nextLevel, blind: "", duration: "15'", note: "" }
                ]
            };
        });
    };
    ...
    const deleteBlindLevel = (id) => {
        setSettings((prev) => {
            ...
            return {
                ...prev,
                blindLevels: prev.blindLevels.filter((item) => item.id !== id)
            };
        });
    };
        ...
        <div className="stack">
            ...
            {subTab === "Settings" && (
                <div className="card">
                    ...
                    <div className="form-grid form-grid-4">
                        <label>
                            ...
                            <input
                                ...
                                onChange={(e) =>
                                    setSettings((prev) => ({ ...prev, buyIn: Number(e.target.value || 0) }))
                                }
                            />
                        </label>
                    </div>
                    ...
                    <div className="row-actions">
                        <button className="btn btn-primary" onClick={addBlindLevel}>
                            Thêm level
                        </button>
                    </div>
                        ...
                        <tbody>
                            {settings.blindLevels.map((item) => (
                                <tr key={item.id}>
                                    ...
                                    <td>
                                        <button className="btn btn-danger" onClick={() => deleteBlindLevel(item.id)}>
                                            Xóa
                                        </button>
                                    </td>
                                </tr>
